...
if ($ListWifi) {
    ...
    $listCmd = 'cd ' + $ap + ' 2>/dev/null; if [ -f list_wifi.sh ]; then sed -i ''s/\r$//'' list_wifi.sh 2>/dev/null; chmod +x list_wifi.sh; bash list_wifi.sh; else nmcli -t -f SSID,SIGNAL device wifi list 2>/dev/null | while IFS=: read -r s n; do echo "  SSID: [$s]  Signal: ${n}%"; done; fi'
    ssh $sshOpt "$($script:PI_USER)@$($script:PI_HOST)" $listCmd
    Write-Host ""
...
if (-not $SkipSync) {
    try {
        ...
        scp -r $sshOpt "ap_device" "$($script:PI_USER)@$($script:PI_HOST):~/"
        Pop-Location
        ...
        try {
            $fixCmd = 'cd ' + $script:PI_AP_DEVICE_PATH + ' && for f in connect_camera_wifi.sh run_relay.sh list_wifi.sh start_relay.sh relay_stream.py; do tr -d ''\r'' < "$f" > "$f.n" 2>/dev/null && mv "$f.n" "$f"; done; for f in config/camera_wifi.conf; do [ -f "$f" ] && tr -d ''\r'' < "$f" > "$f.n" 2>/dev/null && mv "$f.n" "$f"; done; chmod +x connect_camera_wifi.sh run_relay.sh list_wifi.sh start_relay.sh'
            ssh $sshOpt "$($script:PI_USER)@$($script:PI_HOST)" $fixCmd 2>&1 | Out-Null
        } catch { }
    } catch {
...
if (-not $SkipWifi) {
    try {
        $out = ssh $sshOpt "$($script:PI_USER)@$($script:PI_HOST)" "cd $($script:PI_AP_DEVICE_PATH) && chmod +x connect_camera_wifi.sh && ./connect_camera_wifi.sh" 2>&1
...
if (-not $SkipRelay) {
    try {
        ...
        ssh $sshOpt "$($script:PI_USER)@$($script:PI_HOST)" "pkill -9 -f relay_stream.py 2>/dev/null; sleep 2" 2>&1 | Out-Null
        $out = ssh $sshOpt "$($script:PI_USER)@$($script:PI_HOST)" "cd $($script:PI_AP_DEVICE_PATH) && chmod +x start_relay.sh 2>/dev/null; bash start_relay.sh $RelayPort" 2>&1
        ...
        if ($started) {
            try {
                ...
                if ($ver -ne "2") {
                    Write-Host "  [WARN] Relay may be old version (no X-Relay-Version: 2). Restart on Pi: pkill -f relay_stream.py; cd ~/ap_device; bash start_relay.sh $RelayPort" -ForegroundColor Yellow
                }
            } catch {
...
if ($script:PI_HOST -match "^\d+\.\d+\.\d+\.\d+$") {
    ...
    try {
        $piIp = (ssh $sshOpt "$($script:PI_USER)@$($script:PI_HOST)" "hostname -I | awk '{print \$1}'" 2>$null).Trim()
